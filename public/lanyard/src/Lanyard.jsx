...
export default function Lanyard({
  ...
  const handleActive = useCallback((active) => {
    if (wrapperRef.current) wrapperRef.current.classList.toggle('is-active', active);
...
function Band({
  ...
  const pointerNDC = useRef({ x: 2, y: 2 });
  ...
  // 全局监听鼠标，换算成 NDC 供 raycaster 命中检测（canvas 默认不接收指针，必须自己算）
  useEffect(() => {
    const onMove = (e) => {
  ...
  useFrame((state, delta) => {
    if (dragged) {
      ...
      vec.y -= dragged.y;
      vec.z -= dragged.z;
      // 把工牌锁在 iframe 可视区内：拖到边界就像撞墙一样被挡住，拖不出去
      const halfH = Math.tan(((state.camera.fov * Math.PI) / 180) / 2) * state.camera.position.z;
      const halfW = halfH * state.camera.aspect;
      const wallMargin = 1.4;
      vec.x = Math.max(-halfW + wallMargin, Math.min(halfW - wallMargin, vec.x));
      vec.y = Math.max(-halfH + wallMargin, Math.min(halfH - wallMargin, vec.y));
    ...
    // 命中检测：鼠标是否悬停在工牌 mesh 上（或正在拖拽），决定 canvas 是否接收交互
    if (cardGroup.current) {
      raycaster.setFromCamera(pointerNDC.current, camera);
      const hit = raycaster.intersectObject(cardGroup.current, true).length > 0;
      const active = hit || draggedRef.current;
